...
import pandas
# import all from the script so you can use the file directly
from . import hump_timer


# I think this will help clean up some of the code belwow so you dont have 
# to keep calling cursor and connection
class Conn:
    def __init__(self, database="", port = 3306, plug = 'mysql_native_password', host='localhost'):
        self.cursor = None
    ...
    def connect(self, id='root', pwd=''):
        ...
        return mydb
    def isConnected(self, db_conn): # pass mydb to db_conn arguement
        if db_conn:
            self.connected = True
        else:
